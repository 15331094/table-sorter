$(document).ready(function() {
    ...
    for(var i = 0; i < 3; i++) {
        //把每一列的数据放进数组中
        what[i] = $("#todo tbody tr:eq(" + i + ") td:eq(0)").text();
        when[i] = $("#todo tbody tr:eq(" + i + ") td:eq(1)").text();
        location[i] = $("#todo tbody tr:eq(" + i + ") td:eq(2)").text();
        firstName[i] = $("#staff tbody tr:eq(" + i + ") td:eq(0)").text();
        lastName[i] = $("#staff tbody tr:eq(" + i + ") td:eq(1)").text();
        checkIn[i] = $("#staff tbody tr:eq(" + i + ") td:eq(2)").text();        
        //给每一个表头末尾增加一个div三角形
    ...
    */
    ...
    function reset() {
        for(var i = 0; i < 3; i++) {
            $("#todo tbody tr:eq(" + i + ") td:eq(0)").text(what[i]);
            $("#todo tbody tr:eq(" + i + ") td:eq(1)").text(when[i]);
            $("#todo tbody tr:eq(" + i + ") td:eq(2)").text(location[i]);

            $("#staff tbody tr:eq(" + i + ") td:eq(0)").text(firstName[i]);
            $("#staff tbody tr:eq(" + i + ") td:eq(1)").text(lastName[i]);
            $("#staff tbody tr:eq(" + i + ") td:eq(2)").text(checkIn[i]);  
        }
    }
